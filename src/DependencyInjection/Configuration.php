        ...
        $treeBuilder->getRootNode()
            ->children()
                ...
                ->enumNode('persistence')
                    ->info('Persistence adapter: "doctrine" (default), "cache", or a custom service ID')
                    ->values(['doctrine', 'cache'])
                    ->defaultValue('doctrine')
                ->end()
                ->scalarNode('persistence_service')
                    ->info('Custom service ID for the repository when using a custom persistence adapter')
                    ->defaultNull()
                ->end()
                ->arrayNode('notifier')
